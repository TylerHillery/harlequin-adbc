...
CONN_STR = "grpc+tls://localhost:31337"
DRIVER_TYPE = "flightsql"
DB_KWARGS_STR = "username=flight_username;password=flight_password;adbc.flight.sql.client_option.tls_skip_verify=true"  # noqa: E501


def test_connect() -> None:
    conn = HarlequinAdbcAdapter(
        conn_str=(CONN_STR,), driver_type=DRIVER_TYPE, db_kwargs_str=DB_KWARGS_STR
    ).connect()
...
def test_connect_raises_connection_error() -> None:
    with pytest.raises(HarlequinConnectionError):
        _ = HarlequinAdbcAdapter(
            conn_str=("foo",), driver_type=DRIVER_TYPE, db_kwargs_str=DB_KWARGS_STR
        ).connect()
